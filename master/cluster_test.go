...
func TestCreateVolWithUUID(t *testing.T) {
	...
	t.Run("create volume with same UUID but different name - should return error", func(t *testing.T) {
		req := &createVolReq{
		...
		_, err = server.cluster.createVol(req)
		require.Error(t, err)
		require.Contains(t, err.Error(), "volume name mismatch")
		t.Logf("Expected error for UUID mismatch with different name: %v", err)
	})
